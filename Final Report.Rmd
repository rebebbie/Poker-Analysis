...
For clarity, we summarize the basic rules (Texas hold 'em, 2025):

1.     **Hole cards and community cards.** Each player has two hole cards that remain hidden from their opponents. Five community cards are dealt face-up and shared by all players.

2.     **Game flow.** The game begins with each player receiving two private cards. Then there are four rounds of betting, where the community cards are gradually revealed: pre-flop, flop (the first three cards), turn (the fourth card), and river (the fifth card). If two or more players remain after the final betting round, the game proceeds to a showdown, where players reveal their private cards and determine the winner.

3.     **Betting Options.** During each betting round, players can choose from several actions: bet (place an opening bet), check (bet nothing), raise (increase the size of an existing bet), call (match the current bet or raise), fold (discard hand and exit the round).

4.     **Hand Values.** The strength of a player’s hand is determined by the best five-card combination they can make using their two hole cards and the five community cards.
...
![](images/clipboard-2942074169.png){width="559"}

![](images/clipboard-901868597.png)

The above pictures are from ***Using Bayesian Networks to Model a Poker Player*****, (Andrew Haiberg, UCSD)**

To illustrate this in a real game, let’s take a heads-up match in Texas Hold 'Em. One player has Q♠ (Queen of Spades) and 8♣ (Eight of Clubs) as their hole cards. The community cards that come out during the flop, turn, and river are 6♠ (Six of Spades), 7♠ (Seven of Spades), 10♣ (Ten of Clubs), and J♦ (Jack of Diamonds). With this setup, the player could complete a straight if the final card turns out to be an 8 or 9.
...
$$
P(\text{Heavy Bet}) = P(\text{Heavy Bet}\mid\text{Strong Hand})\times P(\text{Strong Hand}) + P(\text{Heavy Bet}\mid\text{Weak Hand})\times P(\text{Weak Hand})\\
= (0.85 \times 0.7) + (0.4 \times 0.3)\\ = 0.715
$$

$$
P(\text{Strong Hand} \mid \text{Heavy Bet}) = \frac{P(\text{Heavy Bet} \mid \text{Strong Hand}) \times P(\text{Strong Hand})}{P(\text{Heavy Bet})}\\=\frac{0.85\times0.7}{0.715}\approx0.833
$$

As a result, the updated belief is that there’s an 83.3 percent chance the opponent has a strong hand. This change in perspective really impacts how the player decides to act. If the opponent does have a strong hand, they might be holding a straight or a flush, which would probably beat the player’s hand. In that scenario, folding could be the best move. However, if the player still thinks there’s a chance the opponent is bluffing—despite the high likelihood of a strong hand—they might decide to call or even raise, depending on how much risk they’re willing to take and their overall strategy.
...
|----------|-----------|-----------------------|
| Pre-Flop | Raise     | N/A                   |
| Flop     | Large Bet | [J♦, 10♦, 9♠]         |
| Turn     | Small Bet | [J♦, 10♦, 9♠, 3♣]     |
| River    | All-in    | [J♦, 10♦, 9♠, 3♣, Q♦] |
...
$$P(A_{pf} = \text{Raise}\mid \text{Prior}) $$= [Strong: 0.7, Medium: 0.5, Weak: 0.1]​. We then apply Bayes Rule as covered by previous section​.​

Post-Preflop HS (rounded): [S: 0.44, M: 0.52, W: 0.04]​. In other phases, repeat using new HS beliefs, dynamically changing these Hand Strength Beliefs.​ Large Bet given a Flop with high potential of straight and flush means likelihood of Strong Hand​
